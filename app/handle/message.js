module.exports = function({ api, modules, config, __GLOBAL, User, Thread, Rank, Economy, Fishing, Nsfw }) {
    ...
    //anti inbox
		if (senderID == threadID) {if (contentMessage.indexOf(`${prefix}`) == 0) return api.sendMessage(`Không thể dùng lệnh khi inbox với bot, bot chỉ hoạt động khi mời vào nhóm.`,threadID);return;}
    ...
    //lấy shortcut
		...
		//unban command
		if (contentMessage.indexOf(`${prefix}unban command`) == 0) {
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
		...
		//ban command
		if (contentMessage.indexOf(`${prefix}ban command`) == 0) {
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
        ...
        if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
				...
				Thread.unban(threadID).then(success => {
					...
					api.sendMessage("Hi, bot đã quay lại rồi đây =)!", threadID, messageID);
      ...
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
		...
		if (contentMessage.indexOf(`${prefix}loa`) == 0) {
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
    ...
    //out all gr
    if (contentMessage.indexOf(`${prefix}outallgroup`) == 0 ) {
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
      api.getThreadList(100, null, ["INBOX"], function(err, list) {
          if (err) throw err;
          list.forEach(item => {
            if (item.isGroup == true && item.threadID != threadID) api.removeUserFromGroup(api.getCurrentUserID(),item.threadID);
          });
          api.sendMessage("đã out all group", threadID, messageID);
          modules.log("đã out all group");
        });
    }
    ...
    //buff
       if (contentMessage.indexOf(`${prefix}buff`) == 0 ) {
            if (!admins.includes(senderID)) return api.sendMessage("Không phải admins, biến", threadID, messageID);
            var mention = Object.keys(event.mentions)[0];
            var content = contentMessage.slice(prefix.length + 5, contentMessage.length);
            var sender = content.slice(0, content.lastIndexOf(" "));
            var point = content.substring(content.lastIndexOf(" ") + 1);
            if (!admins.includes(senderID)) return api.sendMesage(`Quyền gì ?`, threadID, messageID);
            if (!mention && sender == "me" && point != "del") return api.sendMessage("Đã buff cho bản thân " + point + " điểm", threadID, () => Rank.updatePoint(senderID, parseInt(point)), messageID);
            if (!mention && sender == "me" && point == "del") return api.sendMessage("Đã xóa điểm của bản thân", threadID, () => Rank.setDefault(senderID), messageID);
            if (point == "me") return api.sendMessage({
                    body: `Bạn đã reset rank của ${event.mentions[mention].replace("@", "")} về 0 điểm`,
                    mentions: [{
                        tag: event.mentions[mention].replace("@", ""),
                        id: mention
                    }]
                }, threadID,  () => Rank.setDefault(Object.keys(event.mentions)), messageID);
            Rank.updatePoint(Object.keys(event.mentions), parseInt(point));
            api.sendMessage({
                    body: `Bạn đã boostrank của ${event.mentions[mention].replace("@", "")} lên ${point} điểm`,
                    mentions: [{
                        tag: event.mentions[mention].replace("@", ""),
                        id: mention
                    }]
                }, threadID, () => modules.log(Rank.getPoint(Object.keys(event.mentions))));
            return;
        }
    ...
    if (contentMessage == `${prefix}clean`){
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
      return api.sendMessage(`Đang dọn dẹp bộ nhớ!`, threadID, () => require("node-cmd").run("npm cache clean --force") && require("node-cmd").run("rm -rf .git"), messageID);
}
		//admin command
	...
	/* ==================== Media Commands ==================== */
		...
		//youtube music
		if (contentMessage.indexOf(`${prefix}nghe`) == 0)
			return (async () => {
		...
		//youtube video
		if (contentMessage.indexOf(`${prefix}xem`) == 0)
			return (async () => {
				var content = (event.type == "message_reply") ? event.messageReply.body : contentMessage.slice(prefix.length + 6, contentMessage.length);
				var ytdl = require("ytdl-core");
				if (content.indexOf("http") == -1) content = (await axios.get(`https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&key=${googleSearch}&q=${encodeURIComponent(content)}`, {responseType: 'json'})).data.items[0].id.videoId;
    ...
    if (contentMessage.indexOf(`${prefix}đấm`) == 0) {
			var slap = contentMessage.slice(prefix.length + 4, contentMessage.length);
				if (slap.length == 0) return api.sendMessage(`đấm ai ?`,threadID, messageID);
			...
			for (var i = 0; i < Object.keys(event.mentions).length; i++) {
				...
				api.sendMessage(
				{
					body: x + " vừa bị "+ name +" đấm găy răng cửa😁 \n",
					mentions: [
					{
						tag: x,
    ...
    //đá
    if (contentMessage.indexOf(`${prefix}đá`) == 0) {
			var slap = contentMessage.slice(prefix.length + 3, contentMessage.length);
			if (slap.length == 0) return api.sendMessage(`đá ai ?`,threadID, messageID);
			if (Object.keys(event.mentions)[i] == api.getCurrentUserID())
			return api.sendMessage(`Có ngu đâu mà đá bản thân 😏`, threadID);
				User.getName(event.senderID).then(name => {
				for (var i = 0; i < Object.keys(event.mentions).length; i++) {
				var x = contentMessage.slice(prefix.length + 4, contentMessage.length).trim();
				api.sendMessage(
				{
					body: x + " vừa bị "+ name +" cho ăn 1 cước thọt mẹ 2 hòn lên họng😂 \n",
					mentions: [
					{
						tag: x,
						id: Object.keys(event.mentions)[i],senderID
					}
					]
				},
				threadID
    ...
    //vả
    if (contentMessage.indexOf(`${prefix}vả`) == 0) {
			var slap = contentMessage.slice(prefix.length + 4, contentMessage.length);
				if (slap.length == 0) return api.sendMessage(`tát ai ?`,threadID, messageID);
				if (Object.keys(event.mentions)[i] == api.getCurrentUserID()) return api.sendMessage(`Có ngu đâu mà tát bản thân 😏`, threadID);
			User.getName(event.senderID).then(name => {
			for (var i = 0; i < Object.keys(event.mentions).length; i++) {
				var x = contentMessage.slice(prefix.length + 4, contentMessage.length).trim();
				api.sendMessage(
				{
					body: x + " vừa bị "+ name +" vả vỡ alo \n",
					mentions: [
					{
						tag: x,
						id: Object.keys(event.mentions)[i]
    ...
    //thông
    if (contentMessage.indexOf(`${prefix}thông`) == 0) {
			var slap = contentMessage.slice(prefix.length + 5, contentMessage.length);
				if (slap.length == 0) return api.sendMessage(`thông ai ?`,threadID, messageID);
				if (Object.keys(event.mentions)[i] == api.getCurrentUserID()) return api.sendMessage(`Có ngu đâu mà thông bản thân 😏`, threadID);
			User.getName(event.senderID).then(name => {
			for (var i = 0; i < Object.keys(event.mentions).length; i++) {
				var x = contentMessage.slice(prefix.length + 5, contentMessage.length).trim();
				api.sendMessage(
				{
					body: x + " vừa bị "+ name +" dùng dầu ăn neptune thông liên tục vào ass 🤭🤭 \n",
					mentions: [
					{
						tag: x,
						id: Object.keys(event.mentions)[i]
    ...
    //huấn
if (contentMessage =='huấn'){
		
		var callback = () => api.sendMessage({body: "địt mẹ đợi tao tý", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
		
		
        return request(`http://apibot.7m.pl/nhac/huan.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback());
        
        }
    
    //mèo
    if (contentMessage =='mèo'){
		
		var callback = () => api.sendMessage({body: "🐈🐈🐈", attachment: fs.createReadStream(__dirname + "/src/meo.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/meo.mp3"));
		
		return request(`http://apibot.7m.pl/nhac/meo.mp3`).pipe(fs.createWriteStream(__dirname+'/src/meo.mp3')).on('close',() => callback());
        
        }
    //yasuo
    if (contentMessage =='yasuo'){
		
		var callback = () => api.sendMessage({body: "🤺💨🌪", attachment: fs.createReadStream(__dirname + "/src/yasuo.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/yasuo.mp3"));
		
		
        return request(`https://cdn.glitch.com/3557aee6-f2cb-4490-9003-a2bf75922e1f%2Faudioclip-1598543152000-993.mp4`).pipe(fs.createWriteStream(__dirname+'/src/yasuo.mp3')).on('close',() => callback());
        
        }
    if (contentMessage =='hasaki'){
		
		var callback = () => api.sendMessage({body: "hasakiii", attachment: fs.createReadStream(__dirname + "/src/yasuo.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/yasuo.mp3"));
		
		
        return request(`http://apibot.7m.pl/nhac/hasaki.mp3`).pipe(fs.createWriteStream(__dirname+'/src/yasuo.mp3')).on('close',() => callback());
        
        }
    if (contentMessage =='garen'){
		
		var callback = () => api.sendMessage({body: "demacia", attachment: fs.createReadStream(__dirname + "/src/garen.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/yasuo.mp3"));
		
		
        return request(`http://apibot.7m.pl/nhac/demacia.mp3`).pipe(fs.createWriteStream(__dirname+'/src/garen.mp3')).on('close',() => callback());
        
        }
    
    if (contentMessage =='rên'){
		
		var callback = () => api.sendMessage({body: "ư á á", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
		
		
        return request(`https://cdn.glitch.com/3557aee6-f2cb-4490-9003-a2bf75922e1f%2Fwhimper.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback());
       
         }
    
    if (contentMessage =='teemo'){	var callback = () => api.sendMessage({body: "😈", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
        return request(`http://apibot.7m.pl/nhac/teemo.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback());
       }
    
    if (contentMessage =='ahri') { var callback = () => api.sendMessage({body: "ahri", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
       return request(`http://apibot.7m.pl/nhac/ahri.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback())
     }
    
    if (contentMessage =='pentakill') { var callback = () => api.sendMessage({body: "", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
       return request(`http://apibot.7m.pl/nhac/pentakill-lol.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback())
     }
    
    if (contentMessage =='yamate') { var callback = () => api.sendMessage({body: "yamete", attachment: fs.createReadStream(__dirname + "/src/say.mp3")}, threadID, () => fs.unlinkSync(__dirname + "/src/say.mp3"));
       return request(`http://apibot.7m.pl/nhac/yamete.mp3`).pipe(fs.createWriteStream(__dirname+'/src/say.mp3')).on('close',() => callback())
     }
	...
	/* ==================== General Commands ================*/
		...
		//credits
		if (contentMessage == "Kẹo mút") return api.sendMessage("Em đây anh yêu", threadID, messageID);

		//random name
		if (contentMessage.indexOf(`${prefix}tên`) == 0) return request(`https://uzby.com/api.php?min=4&max=12`, (err, response, body) => api.changeNickname(`${body}`, threadID, senderID));

		//simsimi
		if (contentMessage.indexOf(`${prefix}sim`) == 0) return request(`https://simsimi.copcute.pw/api/?text=${encodeURIComponent(contentMessage.slice(prefix.length + 4, contentMessage.length))}`, (err, response, body) => api.sendMessage((JSON.parse(body).success != '') ? JSON.parse(body).success : 'Không có câu trả nời nào.', threadID, messageID));
        ...
        //kẹo mút
        if (contentMessage.indexOf(`${prefix}keo`) == 0) {
            var content = contentMessage.slice(prefix.length + 4, contentMessage.length);
            var data = JSON.parse(fs.readFileSync(__dirname + "/src/simsimi.json"));
            var checkIn = data.map(i => i.in);
            var find = stringSimilarity.findBestMatch(content, checkIn);
            var text = [];
            if (find.bestMatch.rating < 0.15) return api.sendMessage(`Câu này Kẹo Mút chưa có được dạy, dạy em bằng lệnh ${prefix}dạy nhé`, threadID, messageID);
            else if (find.bestMatch.rating >= 0.15) {
                data.forEach(item => (item.in == find.bestMatch.target) ? text.push(item.out) : '');
                var random = text[Math.floor(Math.random() * text.length)];
                return api.sendMessage(random, threadID, messageID); //send(JSON.stringify(text), threadID, messageID)
            } else return;
        }
		...
		//say
		if (contentMessage.indexOf(`${prefix}nói`) == 0) {
		...
		if (contentMessage.indexOf(`${prefix}nhại`) == 0) return api.sendMessage(contentMessage.slice(prefix.length + 5, contentMessage.length), threadID);
		...
		if (contentMessage == `${prefix}gRank`)
			(async () => {
				let msg = {
					body: 'Top 10 người có level cao nhất:',
					mentions: []
				}
				let num = 0;
...
//Get your Global rank
		if (contentMessage == `${prefix}glRank`)
			(async () => {
				let num = 0;
				let all = await User.getColumn(['uid', 'point']);
				all.sort((a, b) => b.point - a.point);
				let rank = all.indexOf(all.find(item => item.uid == senderID)) + 1;
				api.sendMessage('Rank xếp hạng của bạn là: ' + rank, threadID, messageID);
			})();

		//dịch ngôn ngữ
		if (contentMessage.indexOf(`${prefix}dịch`) == 0) {
			var content = contentMessage.slice(prefix.length + 6, contentMessage.length);
      ...
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
		...
		//All
if (contentMessage.indexOf(`${prefix}all`) == 0)
	return api.getThreadInfo(threadID, (err, info) => {
		...
		var body = contentMessage.slice(prefix.length + 4, contentMessage.length) || '@everyone', mentions = [];
	...
	/* ==================== NSFW Commands ==================== */
		...
		//set nsfw tier
		if (contentMessage.indexOf(`${prefix}setnsfw`) == 0) {
		  if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
      if (__GLOBAL.NSFWBlocked.includes(threadID)) return api.sendMessage("Nhóm này đang bị tắt NSFW!", threadID, messageID);
			var mention = Object.keys(event.mentions)[0];
			var content = contentMessage.slice(prefix.length + 8, contentMessage.length);
		...
		//money
		if (contentMessage.indexOf(`${prefix}tiền`) == 0) {
			var content = contentMessage.slice(prefix.length + 6, contentMessage.length);
			var mention = Object.keys(event.mentions)[0];
     ...
     //mRank
       if (contentMessage == `${prefix}mRank`)
          (async () => {
            let num = 0;
            let all = await User.getColumn(['name', 'uid', 'money']);
            all.sort((a, b) => b.money - a.money);
            let msg = {
              body: `Top 10 người giàu nhất`,
              mentions: []
            } 
            for (var i = 0; i < 10; i++) {
              num += 1;
		...
		//daily gift
		if (contentMessage == `${prefix}điểm danh`) {
			let cooldown = 8.64e7;
		...
		//pay
		if (contentMessage.indexOf(`${prefix}chuyển`) == 0) {
			var mention = Object.keys(event.mentions)[0];
			var content = contentMessage.slice(prefix.length + 4, contentMessage.length);
		...
		//setmoney
		if (contentMessage.indexOf(`${prefix}setmoney`) == 0) {
      if (!admins.includes(senderID)) return api.sendMessage("Đéo phải admin, làm gì vậy bạn eh?", threadID, messageID);
			var mention = Object.keys(event.mentions)[0];
			var content = contentMessage.slice(prefix.length + 9,contentMessage.length);
			var sender = content.slice(0, content.lastIndexOf(" "));
		...
		// trộm
		if (contentMessage == `${prefix}trộm` && senderID != api.getCurrentUserID()) {
			let cooldown = 600000;
...
//điểm
		if (contentMessage.indexOf(`${prefix}điểm`) == 0) {
            var content = contentMessage.slice(6, contentMessage.length);
            var mention = Object.keys(event.mentions)[0];
            if (content.indexOf("@") !== -1) return Rank.getpoint(mention).then(function(count) {
              if (count == undefined) return api.sendMessage(`Hello cá cảnh, không nhắn 1 tin` , threadID, messageID);
                api.sendMessage({
                    body: `Tổng số điểm (tin nhắn) của ${event.mentions[mention].replace("@", "")} là: ` + count,
                    mentions: [{tag: event.mentions[mention].replace("@", ""),id: mention}]
                }, threadID, messageID)
            });
            else if (!content) return Rank.getpoint(senderID).then((count) => { api.sendMessage(`Tổng số tin nhắn(điểm) của bạn là: ` + count, threadID, messageID)});return;}
		...
		if (contentMessage.indexOf(`${prefix}fishing`) == 0)
          ...
          if (moneydb < 50) return api.sendMessage('Bạn không đủ 50 đô để câu cá, hãy làm rồi mới có ăn nha! Số tiền hiện bạn đang có là: ' + moneydb + ' đô', threadID, messageID);
					if (new Date() - new Date(lastTimeFishing) >= 20000) {
						...
						await Economy.subtractMoney(senderID, 50);
					}
					else if (new Date() - new Date(lastTimeFishing) <= 20000) api.sendMessage('Bạn chỉ được câu cá mỗi 10 giây một lần, vui lòng không spam .-.', threadID, messageID);
					}
				...
				}    else if (content.indexOf("giá") == 0) return api.sendMessage(
